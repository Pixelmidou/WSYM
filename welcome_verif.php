...
if ($con->connect_error) {
    ...
    session_start();
    if (!isset($_SESSION['deposit_verif']) && !isset($_SESSION['withdraw_verif']) && !isset($_SESSION['wire_verif']) && !isset($_SESSION['ticket_verif']) && !isset($_SESSION['dephis_show'])) {
        if (empty($_SESSION['deposit_verif']) || $_SESSION['deposit_verif'] === "" && empty($_SESSION['withdraw_verif']) || $_SESSION['withdraw_verif'] === "" && empty($_SESSION['wire_verif']) || $_SESSION['wire_verif'] === ""  && empty($_SESSION['ticket_verif']) || $_SESSION['ticket_verif'] === "" && empty($_SESSION['dephis_show']) || $_SESSION['dephis_show'] === "") {
            session_destroy();
    ...
    if (isset($_SESSION['dephis_show'])) {
        ...
        if ($dephis_show === "success") {
            ...
            $dephis_case = "initial";
            $dephis_query = mysqli_query($con,"SELECT username,deposit_date,deposit_amount FROM deposit WHERE username LIKE '%$user_username%' LIMIT $start , $rowsperpage");
            $dephis_query_date_desc = mysqli_query($con,"SELECT username,deposit_date,deposit_amount FROM deposit WHERE username LIKE '%$user_username%' ORDER BY deposit_date DESC LIMIT $start , $rowsperpage");
            $dephis_query_date_asc = mysqli_query($con,"SELECT username,deposit_date,deposit_amount FROM deposit WHERE username LIKE '%$user_username%' ORDER BY deposit_date ASC LIMIT $start , $rowsperpage");
            $dephis_query_amount_desc = mysqli_query($con,"SELECT username,deposit_date,deposit_amount FROM deposit WHERE username LIKE '%$user_username%' ORDER BY deposit_amount DESC LIMIT $start , $rowsperpage");
            $dephis_query_amount_asc = mysqli_query($con,"SELECT username,deposit_date,deposit_amount FROM deposit WHERE username LIKE '%$user_username%' ORDER BY deposit_amount ASC LIMIT $start , $rowsperpage");
        }
        ...
        <div class="container1">
            <form method="post" class="container2">
                ...
                <div class="d-flex gap-3 mt-auto mb-auto">
                    <input type="submit" class="x but text-center" name="dephisfi_submit" value="Filter">
                    <input type="submit" id="but" name="depback" class="x but text-center" value="Back to the welcome page" formnovalidate>
                </div>
        </form>
    </div>
